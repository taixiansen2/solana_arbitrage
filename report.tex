...
\section{结果分析}
基于目前已提取解析完成的 \textbf{57,059} 笔入库交易样本，得出如下初步结论：
\begin{itemize}
    \item \textbf{时间覆盖与活跃度}：分析涵盖了自 2026-03-17 11:04 至 2026-04-05 11:32（UTC）的多个活跃时段。绝大多数交易在非常短的密集区块时间内产生。
    ...
    \item \textbf{头部程序占比}：在已入库数据中，\texttt{pAMMBay6oceH9fJKBRHGP5D4...} 是绝对的主力，共出现 24,660 次（占展开后匹配量的 \textbf{42.5\%}），紧随其后的是 \texttt{9H6tua...} 出现了 20,954 次。
    \item \textbf{方向与资产}：启发式方向推断结果显示高达 45,592 笔交易为 \texttt{unknown}（占比近 80\%），说明交易路径极有可能涉及三边及以上的复杂原子路由套利。最常涉及的核心资产除了主流的 WSOL 外，USDC (\texttt{EPjFW...}) 也是最重要的计价和中转资产（出现在了近 8,000 次净变动中）。
\end{itemize}
