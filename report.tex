...
\subsection{采集速度瓶颈}
在最近的测试中，脚本分析仅 1000 个区块（Slot 区间为 407000000 - 407001000）就耗时高达约 4.5 小时。与以往的以太坊套利检测相比，这是一个极其反常的耗时。若以此速度线性外推，按 Solana 主网一天约产生 20 万个区块计算，提取一个月（约 600 万个区块）的交易数据将需要耗时高达 27,000 小时（约 1125 天），这在实际科研分析中是完全不可接受的。
