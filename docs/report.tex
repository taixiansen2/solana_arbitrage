...
\begin{itemize}
    \item \textbf{极低的聚合器依赖度}：在严密的 MEV 程序筛选下，命中聚合器（\texttt{via\_aggregator}）以及 \texttt{jupiter\_heavy} 依赖度的占比为 \textbf{0.0\%}。这证实了：真正的套利机器人和做市商为了追求速度和微薄利差，绝不使用面向散户的聚合路由，而是自建智能合约直接使用 CPI 调用底层 DEX 进行原子操作。
